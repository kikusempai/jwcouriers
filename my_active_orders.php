...
<html lang="en">
  <head>
    <meta charset="utf-8">
    <title>J.W.Couriers &middot; My active Orders</title>
  ...
  <div class="content">
    <div class="container">
      <?php
        ...
        $userdataq=<<<EOT
          SELECT * FROM `order` WHERE `userid` = '{$_SESSION['userid']}' AND `status` not like 'complete';
EOT;
        ...
        while ($results = mysql_fetch_array($querydata_handle, MYSQL_NUM)) { 
            echo '
                  <tr>
                        ...
                        <th>'.$results[15].'</th>
                        <th>
                          <form id="track" class="form-horizontal" method="post" action="track.php">
                            <input type="hidden" id="parcelnumber" name="number" value="'.$results[0].'">
